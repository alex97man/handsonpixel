...
const allImages = [heroImg, secondaryImg1, secondaryImg2];
const allAlts = ['Pizzeria Oro Hero', 'Pizzeria Oro Detail 1', 'Pizzeria Oro Detail 2'];

export default function PizzeriaOro() {
  ...
  return (
    <div className="w-full relative break-words text-text pb-24">
      <SEO 
        title="PIZZERIAORO.RO | Website Prezentare"
        description="Website de prezentare creat pe platforma Wordpress pentru Pizzeria Oro."
      />
      ...
      <section className="pt-40 md:pt-48 pb-16 px-6 md:px-8 max-w-5xl mx-auto relative z-10 w-full">
        ...
        <motion.div initial="hidden" animate="visible" variants={{ hidden: { opacity: 0 }, visible: { opacity: 1, transition: { staggerChildren: 0.15 } } }}>
          ...
          <motion.h1 variants={fadeUpVariants} className="text-4xl md:text-6xl lg:text-7xl font-black text-text uppercase tracking-normal leading-[0.85] mb-8">
            PIZZERIAORO.RO<span className="text-accent font-['Russo_One'] ml-1">.</span>
          </motion.h1>
        </motion.div>
      </section>
      ...
      <section ref={targetRef} className="px-6 md:px-8 max-w-5xl mx-auto mb-20 relative">
        ...
        >
          <motion.img
            style={{ y, scale: 1.3 }}
            src={heroImg} alt="Pizzeria Oro Showcase" className="w-full h-full object-cover" />
        </motion.div>
      </section>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-12">
          <div className="md:col-span-1">
            ...
            <p className="text-muted text-sm leading-relaxed">
              [PLACEHOLDER DESPRE PROIECT - Adaugă aici o descriere a proiectului pentru Pizzeria Oro.]
            </p>
          </div>
          <div className="md:col-span-1">
      ...
      <section className="px-6 md:px-8 max-w-5xl mx-auto mb-32 grid grid-cols-1 md:grid-cols-2 gap-8">
        <motion.div whileInView={{ opacity: 1, y: 0 }} initial={{ opacity: 0, y: 40 }} viewport={{ once: true }} onClick={() => openLightbox(1)} className="rounded-[2.5rem] overflow-hidden aspect-[4/5] border border-white/10 cursor-zoom-in">
          <img src={secondaryImg1} alt="Pizzeria Oro Detail 1" className="w-full h-full object-cover hover:scale-105 transition-transform duration-700" />
        </motion.div>
        <motion.div whileInView={{ opacity: 1, y: 0 }} initial={{ opacity: 0, y: 40 }} viewport={{ once: true }} onClick={() => openLightbox(2)} className="rounded-[2.5rem] overflow-hidden aspect-[4/5] border border-white/10 cursor-zoom-in">
          <img src={secondaryImg2} alt="Pizzeria Oro Detail 2" className="w-full h-full object-cover hover:scale-105 transition-transform duration-700" />
        </motion.div>
      </section>

      {/* 5. NEXT PROJECT BUTTON */}
      <section className="px-6 md:px-8 max-w-5xl mx-auto flex justify-end">
        <Link to="/portofoliu" className="group inline-flex items-center gap-6 p-8 rounded-[2rem] border border-white/10 bg-white/[0.02] hover:bg-white/[0.05] transition-all hover:border-accent/50 w-full md:w-auto">
          <div className="text-left">
            <div className="text-accent text-[10px] font-bold uppercase tracking-widest mb-2">VEZI TOATE PROIECTELE</div>
