...
import blue2 from '../../assets/BLUE/BLUE2.jpg';
import blue3 from '../../assets/BLUE/BLUE3.jpg';
...
const allImages = [heroImg, blue1, blue2, blue3];
const allAlts = ['Blue Hero', 'Blue Detail 1', 'Blue Detail 2', 'Blue Detail 3'];
    ...
    <div className="w-full relative break-words text-text pb-24">
      ...
      <section ref={targetRef} className="px-6 md:px-8 max-w-5xl mx-auto mb-20 relative">
        ...
        >
          <motion.img
            style={{ y, scale: 1.3 }}
            src={heroImg} alt="Blue Project Hero" className="w-full h-full object-cover" />
        </motion.div>
      </section>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-12">
          <div className="md:col-span-1">
            ...
            <p className="text-muted text-sm leading-relaxed">
              Fotografie editorială și 360° pentru o sticlă de apă — fundal negru, lumini albastre, atmosferă dramatică.
            </p>
          </div>
          <div className="md:col-span-1">
      ...
      <section className="px-6 md:px-8 max-w-5xl mx-auto mb-32">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          <motion.div whileInView={{ opacity: 1, y: 0 }} initial={{ opacity: 0, y: 40 }} viewport={{ once: true }} onClick={() => openLightbox(1)} className="rounded-[2.5rem] overflow-hidden aspect-[2/3] border border-white/10 cursor-zoom-in">
            <img src={blue1} alt="Product detail 1" className="w-full h-full object-cover hover:scale-105 transition-transform duration-700" />
          </motion.div>
          <motion.div whileInView={{ opacity: 1, y: 0 }} initial={{ opacity: 0, y: 40 }} viewport={{ once: true }} onClick={() => openLightbox(2)} className="rounded-[2.5rem] overflow-hidden aspect-[2/3] border border-white/10 cursor-zoom-in">
            <img src={blue2} alt="Product detail 2" className="w-full h-full object-cover hover:scale-105 transition-transform duration-700" />
          </motion.div>
          <motion.div whileInView={{ opacity: 1, y: 0 }} initial={{ opacity: 0, y: 40 }} viewport={{ once: true }} onClick={() => openLightbox(3)} className="rounded-[2.5rem] overflow-hidden aspect-[2/3] border border-white/10 cursor-zoom-in">
            <img src={blue3} alt="Product detail 3" className="w-full h-full object-cover hover:scale-105 transition-transform duration-700" />
          </motion.div>
        </div>
      </section>
      ...
      <section className="px-6 md:px-8 max-w-5xl mx-auto mb-32 relative">
        ...
        <motion.div whileInView={{ opacity: 1, y: 0 }} initial={{ opacity: 0, y: 40 }} viewport={{ once: true }} className="w-full rounded-[2.5rem] overflow-hidden aspect-video md:aspect-[21/9] border border-white/10 bg-white relative flex items-center justify-center p-4">
          <iframe src="https://alex97man.sirv.com/alcalia/alcalia.spin" width="100%" height="100%" frameBorder="0" allowFullScreen className="w-full h-full absolute inset-0"></iframe>
        </motion.div>
      </section>
