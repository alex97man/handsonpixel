...
import heroImg from '../../assets/KV-VLT-2025/hero_kv_vlt2025.jpg';
import secondaryImg from '../../assets/KV-VLT-2025/img1_kv_vlt2025.jpg';
import secondaryImg2 from '../../assets/KV-VLT-2025/img2_kv_vlt2025.jpg';
import secondaryImg3 from '../../assets/KV-VLT-2025/img3_kv_vlt2025.jpg';
...
const allImages = [heroImg, secondaryImg, secondaryImg2, secondaryImg3];
const allAlts = ['Key Visual 2025 Hero', 'KV 2025 Detail 1', 'KV 2025 Detail 2', 'KV 2025 Detail 3'];

export default function KVVerde2025() {
  ...
  return (
    <div className="w-full relative break-words text-text pb-24">
      <SEO 
        title="Key Visual Verde la Teatru 2025"
        description="Identitate vizuală și materiale promoționale pentru ediția a 7-a a festivalului Verde la Teatru. Design grafic premium pentru evenimente."
      />
      ...
      <section className="pt-40 md:pt-48 pb-16 px-6 md:px-8 max-w-5xl mx-auto relative z-10 w-full">
        ...
        <motion.div initial="hidden" animate="visible" variants={{ hidden: { opacity: 0 }, visible: { opacity: 1, transition: { staggerChildren: 0.15 } } }}>
          ...
          <motion.h1 variants={fadeUpVariants} className="text-4xl md:text-6xl lg:text-7xl font-black text-text uppercase tracking-normal leading-[0.85] mb-8">
            KEY VISUAL VERDE LA TEATRU 2025<span className="text-accent font-['Russo_One'] ml-1">.</span>
          </motion.h1>
        </motion.div>
      </section>
      ...
      <section ref={targetRef} className="px-6 md:px-8 max-w-5xl mx-auto mb-20 relative">
        ...
        >
          <motion.img
            style={{ y, scale: 1.3 }}
            src={heroImg} alt="Key Visual 2025 Hero" className="w-full h-full object-cover" />
        </motion.div>
      </section>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-12">
          <div className="md:col-span-1">
            ...
            <p className="text-muted text-sm leading-relaxed">
              Key visual și materiale promoționale pentru ediția a 7-a a festivalului Verde la Teatru — badge-uri, program, afișe și declinări digitale.
            </p>
          </div>
          <div className="md:col-span-1">
      ...
      <section className="px-6 md:px-8 max-w-5xl mx-auto mb-32">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          <motion.div whileInView={{ opacity: 1, y: 0 }} initial={{ opacity: 0, y: 40 }} viewport={{ once: true }} onClick={() => openLightbox(1)} className="rounded-[2.5rem] overflow-hidden aspect-[2/3] border border-white/10 cursor-zoom-in">
            <img src={secondaryImg} alt="KV 2025 Detail 1" className="w-full h-full object-cover hover:scale-105 transition-transform duration-700" />
          </motion.div>
          <motion.div whileInView={{ opacity: 1, y: 0 }} initial={{ opacity: 0, y: 40 }} viewport={{ once: true }} onClick={() => openLightbox(2)} className="rounded-[2.5rem] overflow-hidden aspect-[2/3] border border-white/10 cursor-zoom-in">
            <img src={secondaryImg2} alt="KV 2025 Detail 2" className="w-full h-full object-cover hover:scale-105 transition-transform duration-700" />
          </motion.div>
          <motion.div whileInView={{ opacity: 1, y: 0 }} initial={{ opacity: 0, y: 40 }} viewport={{ once: true }} onClick={() => openLightbox(3)} className="rounded-[2.5rem] overflow-hidden aspect-[2/3] border border-white/10 cursor-zoom-in">
            <img src={secondaryImg3} alt="KV 2025 Detail 3" className="w-full h-full object-cover hover:scale-105 transition-transform duration-700" />
          </motion.div>
        </div>
      </section>
      ...
      <section className="px-6 md:px-8 max-w-5xl mx-auto flex justify-end">
        <Link to="/portofoliu/beerstation-ro" className="group inline-flex items-center gap-6 p-8 rounded-[2rem] border border-white/10 bg-white/[0.02] hover:bg-white/[0.05] transition-all hover:border-accent/50 w-full md:w-auto">
